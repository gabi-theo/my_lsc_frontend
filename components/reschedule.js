...
const reschedule = () => {
  let sessionID, sessionDate, absenceID;
  let schoolID = getCookie("selected-school");
  let sessionType;
  ...
  radioButtons.forEach((clickedButton) => {
    clickedButton.addEventListener("click", () => {
      ...
      document.querySelector(".makeups-container").innerHTML = "";
      document.querySelector(".mins-container").innerHTML = "";
      sessionType = clickedButton.value;

      fetch(
        `http://127.0.0.1:8000/api/get_available_make_ups/${schoolID}/${absenceID}/${sessionType}/`,
        {
  ...
  const handleRescheduleData = (
    ...
    data[buttonValue].courses.forEach((course) => {
      ...
      if (course.course_session__id === sessionID) {
        let badgeColor;
        if (course.course_session__available_places_for_make_up_on_site < 2) {
    ...
    if (
      data[buttonValue]["30_mins"].make_up_possible_before_session.status ===
      true
      //   &&
      // data[buttonValue]["30_mins"].make_up_possible_before_session.date ===
      //   sessionDate
    ) {
      const sessionContainer = document.createElement("div");
      sessionContainer.className = "form-check my-2";
      sessionContainer.innerHTML = `
                    ...
                    <label class="btn btn-outline-primary" for="${data[buttonValue]["30_mins"].make_up_possible_before_session.date}-${data[buttonValue]["30_mins"].make_up_possible_before_session.start}">Before in ${data[buttonValue]["30_mins"].make_up_possible_before_session.date} at ${data[buttonValue]["30_mins"].make_up_possible_before_session.start}</label>
                      `;
      document.querySelector(".mins-container").appendChild(sessionContainer);
    }

    if (
      data[buttonValue]["30_mins"].make_up_possible_after_session.status ===
      true
      //    &&
...
//       clickedButton.addEventListener("click", () => {
//         fetch(
//           `http://127.0.0.1:8000/api/get_available_make_ups/b373fccd-d2da-4e8e-b156-0482e03ec88c/onl`
//         )
//           .then((response) => {
//             if (!response.ok) {
//               throw new Error("Available Makeups Network Response was not OK");
